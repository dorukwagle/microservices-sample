...
export class AllExceptionsFilter implements ExceptionFilter {
  constructor(private readonly logger: LoggerService) {}
    ...
    const status = isHttpException
      ...
      // also check if the error is axios communication error
      const isAxiosError = exception instanceof AxiosError;
      const errorResponse = isHttpException
        ? exception.getResponse()
       : { message: isAxiosError ? `Service communication error. ${request.url} currently unreachable` : 'Internal server error' };
    if (status >= 500) {
      this.logger.error(
        JSON.stringify({
